...
void main() {
  test('Liam card → recognised → scan → correct answer → rewards → clear',
      () async {
    ...
    expect(container.read(kioskControllerProvider).state,
        KioskState.studentRecognised);
    expect(container.read(kioskControllerProvider).student?.firstName, 'Liam');
    final startingXp = container.read(kioskControllerProvider).student!.totalXp;
    ...
    expect(
      container.read(kioskControllerProvider).classification?.predictedCategory,
  ...
  test('an incorrect answer awards no points and never goes negative',
      () async {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    ...
    await controller.readCard(MockSeedData.liamCardUid);
    final pointsBefore =
        container.read(kioskControllerProvider).student!.availablePoints;
    ...
    expect(state.student!.availablePoints, greaterThanOrEqualTo(pointsBefore));
  ...
  test('a low-confidence item routes to General Waste', () async {
    ...
    expect(container.read(kioskControllerProvider).routedCategory,
